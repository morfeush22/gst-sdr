...
static GstPadProbeReturn UnlinkCall(GstPad *pad, GstPadProbeInfo *info, gpointer container_ptr) {
	...
	PULSE_SINK_CAST(sink_data->abstract_sink_)->Finish();
...
PulseSink::~PulseSink() {
	delete PULSE_SINK_DATA_CAST(data_->sink_data_);
	delete data_;
}
...
void PulseSink::InitSink(void *other_data) {
	...
	PulseSinkHelpers::Data *sink_data = PULSE_SINK_DATA_CAST(data_->sink_data_);
...
void PulseSink::Finish() {
	gboolean connected = gst_object_has_ancestor(reinterpret_cast<GstObject *>(PULSE_SINK_DATA_CAST(data_->sink_data_)->sink_),
	...
	gst_pad_add_probe(PULSE_SINK_DATA_CAST(data_->sink_data_)->teepad_, GST_PAD_PROBE_TYPE_IDLE, UnlinkCall, &data_, NULL);
}
